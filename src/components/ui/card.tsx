    ...
    <CardInnerBorder>
      {Icon && (
        <span className="inline-flex items-center justify-center rounded-md bg-slate-100/10 p-3 shadow-lg dark:bg-slate-500/10">
          <Icon
            className="fill-primary/30 text-primary h-6 w-6"
            aria-hidden="true"
          />
        </span>
      )}
...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, variant, data, children, ...props }, ref) => {
    ...
    if (!result.success) {
      console.error('🚀 ~ Card:', result.error);
    }
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'border-primary/30 dark:border-primary/15 dark:bg-dark-light/30 relative scale-100 rounded-lg border bg-white/10 p-3 shadow-lg transition-transform duration-500 hover:scale-105',
          className,
        )}
        {...props}
      >
